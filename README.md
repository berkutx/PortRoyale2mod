...
| 17 `MIPMCONT → IMSLDXT1/3/5` | SLD-compressed DXT1/3/5 | yes | `nation00..03.aim` world-map flag sources |
...
Encoding on write (city_editor / atlasEditor): class 17 → MIPMCONT/IMSLD32
raw; class 18 single-image (e.g. nation flags) → TILEDIM + native IMSLD32
tile with the original `IHHW` trailer preserved (MIPMCONT crashes the 2D
loader). TGARES → in-place payload rewrite. Atlas pages → re-encoded per
tile in their original format.
